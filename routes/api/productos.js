...
router.post("/", (req, res) => {
  if (req.body.title && req.body.price && req.body.thumbnail) {
    ...
    res.status(200).json({ error: "La peticion no es correcta" });
  }
});
...
router.put("/:id", (req, res) => {
  ...
  if (isNaN(parseInt(req.params.id))) {
    reqOk = false;
    msgError = "El parametro no es un numero";
  } else {
    pos = getPostById(parseInt(req.params.id));
    if (pos === -1) {
      reqOk = false;
      ...
      reqOk = false;
      msgError = "La peticion no es correcta";
    }
  }
  if (reqOk) {
